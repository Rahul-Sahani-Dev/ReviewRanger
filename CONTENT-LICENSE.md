# Content license

The **content** of ReviewRanger — the puzzles, the glossary, the primer, and all
learner-facing words in [`content.js`](content.js) — is licensed under the
**Creative Commons Attribution-ShareAlike 4.0 International License (CC BY-SA 4.0)**.

Full license text: https://creativecommons.org/licenses/by-sa/4.0/legalcode
Human-readable summary: https://creativecommons.org/licenses/by-sa/4.0/

You are free to:

- **Share** — copy and redistribute the content in any medium or format.
- **Adapt** — remix, transform, and build upon it, for any purpose, even commercially.

Under these terms:

- **Attribution** — you must give appropriate credit using the attribution string
  below, link to the license, and indicate if changes were made.
- **ShareAlike** — if you remix or build upon the content, you must distribute your
  contributions under this same CC BY-SA 4.0 license.

## Attribution string

> ReviewRanger by YouCantBearMe and contributors.

## Note on the code

The **code** of ReviewRanger (everything that is not learner-facing content) is
licensed separately under **GPL-3.0** — see [`LICENSE`](LICENSE).

Copyright © YouCantBearMe and contributors. Because ReviewRanger runs entirely on the
device with no server, there is no data controller.
